...
// Processo principale
PROCESS(watering_lance_process, "Irrigation Controller (Watering Lance)");
AUTOSTART_PROCESSES(&watering_lance_process);
...
/*-----------------------------------------------------------------------------*/
PROCESS_THREAD(watering_lance_process, ev, data)
{
  ...
  while(!registered) {
    ...
    const char msg[] = "{\"device\":\"watering_lance\"}";
    coap_set_payload(request, (uint8_t *)msg, sizeof(msg) - 1);

    COAP_BLOCKING_REQUEST(&server_ep, request, client_chunk_handler);
  }

  LOG_INFO("Irrigation controller registered and ready\n");
